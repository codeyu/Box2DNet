	...
	public class BroadPhaseTest : Test
	{
		public class Callback : PairCallback
		{
			public override object PairAdded(object proxyUserData1, object proxyUserData2)
			{
				...
				Box2DNetDebug.Assert(id1 < BroadPhaseTest.k_actorCount);
				Box2DNetDebug.Assert(id2 < BroadPhaseTest.k_actorCount);

				Box2DNetDebug.Assert(_test._overlaps[id1][id2] == false);
		...
		private void Validate()
		{
			...
			for (int i = 0; i < k_actorCount; ++i)
			{
				...
				for (int j = i + 1; j < k_actorCount; ++j)
				{
					...
					if (overlap)
					{
						Box2DNetDebug.Assert(_overlaps[Array.IndexOf(_actors, actor1)][Array.IndexOf(_actors, actor2)] == true);
					}
				}
			}

			Box2DNetDebug.Assert(_overlapCount >= _overlapCountExact);
		}
	}
}
