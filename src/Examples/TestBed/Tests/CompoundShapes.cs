	...
	public class CompoundShapes : Test
	{
		public CompoundShapes()
		{
...

			{
				...
				for (int i = 0; i < 10; ++i)
				{
					float x = Box2DNet.Common.Math.Random(-0.1f, 0.1f);
					BodyDef bd = new BodyDef();
					bd.Position.Set(x + 5.0f, 1.05f + 2.5f * i);
					bd.Angle = Box2DNet.Common.Math.Random(-Box2DNet.Common.Settings.Pi, Box2DNet.Common.Settings.Pi);
					Body body = _world.CreateBody(bd);
...

			{
				...
				sd2.SetAsBox(0.25f, 0.5f, new Vec2(0.0f, -0.5f), 0.5f * Box2DNet.Common.Settings.Pi);
				sd2.Density = 2.0f;
			...
			{
				XForm xf1 = new XForm();
				xf1.R.Set(0.3524f * Box2DNet.Common.Settings.Pi);
				xf1.Position = Box2DNet.Common.Math.Mul(xf1.R, new Vec2(1.0f, 0.0f));

				PolygonDef sd1 = new PolygonDef();
				sd1.VertexCount = 3;
				sd1.Vertices[0] = Box2DNet.Common.Math.Mul(xf1, new Vec2(-1.0f, 0.0f));
				sd1.Vertices[1] = Box2DNet.Common.Math.Mul(xf1, new Vec2(1.0f, 0.0f));
				sd1.Vertices[2] = Box2DNet.Common.Math.Mul(xf1, new Vec2(0.0f, 0.5f));
				sd1.Density = 2.0f;

				XForm xf2 = new XForm();
				xf2.R.Set(-0.3524f * Box2DNet.Common.Settings.Pi);
				xf2.Position = Box2DNet.Common.Math.Mul(xf2.R, new Vec2(-1.0f, 0.0f));

				PolygonDef sd2 = new PolygonDef();
				sd2.VertexCount = 3;
				sd2.Vertices[0] = Box2DNet.Common.Math.Mul(xf2, new Vec2(-1.0f, 0.0f));
				sd2.Vertices[1] = Box2DNet.Common.Math.Mul(xf2, new Vec2(1.0f, 0.0f));
				sd2.Vertices[2] = Box2DNet.Common.Math.Mul(xf2, new Vec2(0.0f, 0.5f));
				sd2.Density = 2.0f;
